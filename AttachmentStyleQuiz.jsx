...
/**
 * AttachmentStyleQuiz component - A React component that presents an interactive quiz
 * to determine the user's attachment style based on their responses to relationship scenarios.
 *
 * The quiz consists of multiple-choice questions, each with options that score towards
 * four attachment styles: secure, anxious, avoidant, and fearful-avoidant.
 *
 * @returns {JSX.Element} The quiz interface with welcome, question, and result screens
 */
const AttachmentStyleQuiz = () => {
  ...
  // Quiz questions with scoring for each attachment style
  const questions = [
    {
  ...
  // Attachment style definitions with descriptions and traits
  ...
  /**
   * Calculates the attachment style based on accumulated scores from answers.
   * @returns {string} The attachment style with the highest score
   */
  const calculateResult = () => {
    ...
    const attachmentStyle = Object.keys(scores).find(key => scores[key] === maxScore);
   ...
   * Updates answers state and progresses to next question or result screen.
   * @param {object} option - The selected answer option with text and score
   */
  ...
  /**
   * Resets the quiz to the welcome screen.
   */
  const restart = () => {
    setIsAnimating(true);
    setTimeout(() => {
  ...
  /**
   * Starts the quiz by transitioning to the quiz screen.
   */
  const startQuiz = () => {
    setIsAnimating(true);
    setTimeout(() => {
          ...
          {currentScreen === 'welcome' && (
            <div className="text-center space-y-8">
              ...
              <button
                ...
                aria-label="Start the attachment style quiz"
              >
          ...
          {currentScreen === 'quiz' && (
            <div className="space-y-8">
              <div className="flex justify-between items-center mb-8">
                <div className="flex gap-2" role="progressbar" aria-valuenow={currentQuestion + 1} aria-valuemax={questions.length}>
